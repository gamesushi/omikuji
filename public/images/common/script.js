...
function showText() {
    ...
    // 直接使用数组join，不会产生额外的空行
    const cleanText = lines.join('\n');

    document.getElementById('signText').textContent = cleanText;
    updateState('text');
}
...
function updateState(newState) {
    ...
    // 更新显示状态
    elements.signTextContainer.style.display = state === 'text' ? 'block' : 'none';
    elements.cardContainer.style.display = state === 'card' ? 'block' : 'none';
    elements.homeContainer.style.display = state === 'home' ? 'block' : 'none';
    elements.decodeBtn.style.display = state === 'card' ? 'block' : 'none';
    elements.redrawBtn.style.display = state === 'card' ? 'block' : 'none';
    elements.backBtn.style.display = state === 'text' ? 'block' : 'none';
    elements.shareBtn.style.display = state === 'text' ? 'block' : 'none';
